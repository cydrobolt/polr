...
if (!is_array($auth->islogged())) {
  ...
  <br><br><input type="submit" class="btn-success btn-lg" disabled="disabled"/>
    ...
    if ($isadmin == true) {
        ...
        echo '<div class="tab-pane" id="adminpanel"><br>Polr Links - Limited @ 360:' . $linksadmin . '<br>Polr Users - Limited @ 360:' . $usersadmin . '<br>Debug Variables: <br>Default IP Fetch: ' . $ip . '<br>X-Forwarded-For:' . @$headers['X-Forwarded-For'] . '<br>Forwarded-For' . @$headers['forwarded-for'] . '<script src="ucpjs.js"></script>'; //<br><br>Session Dump:'.$sessiondump.'</div>';
    }
